...
void init() {
	...
	state = 0;
	flareNum = 0;
	memset(origin,0.f,3*sizeof(float)); 
	earth[0] = 0.64f;
	earth[1] = earth[2] = 0.f;
	uploadPos[0] = 0.47f;
...
void setPositionTarget(float target[3], float multiplier) {
	...
	if (minDistanceFromOrigin(target) > 0.32) {
		...
		if(distance(myState,target)<0.15){
		    haulAssTowardTarget(target,2.5);
		}
		else if(distance(myState,target)<0.2){
...
bool destroySouls(float other[12]){
...
for (int i = 0; i < 3; i++) scaledTarget[i] = myState[i] + scalar * (target[i] - myState[i]);
