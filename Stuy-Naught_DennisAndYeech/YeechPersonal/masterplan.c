...
// Problem:
// TakePic_Two: game.alignLine(middlePOI) returns true... but does not take a real photo...
...
void loop() {
	...
	else if (nextFlare <= 30 && state == TakePic_One) {
		state = GO_TO_SHADOW;
		DEBUG(("\nnextFlare: %d\nOH NO IT'S A FLARE!!!!!\n",nextFlare));
	}
	else if (nextFlare <= 20) {
		state = GO_TO_SHADOW;
		DEBUG(("\nnextFlare: %d\nOH NO IT'S A FLARE!!!!!\n",nextFlare));
	}
...
bool inShadow(ZRState other, int t, float target[3]) {

	if (other[0] + other[4] * t > 0) {
		if (fabsf(other[1] + other[5] * t) <= 0.2) {
			if (fabsf(other[2] + other[6] * t) <= 0.2) {
